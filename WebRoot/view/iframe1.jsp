...
<head>
    ...
    window.onload=function(){
        ...
        $("#sub").click(function(){
            ...
            if(nTitle.value.replace(" ","")=="请输入标题"){
            	alert("标题不能为空");
            	return false;
            }
            if(nContent.value.replace(" ","")==""){
            	alert("内容不能为空");
            	return false;
            }
            $.ajax({
                url:'online_sendQues.action?dd='+Math.random(),
                type:'post',
    ...
    <script type="text/javascript">
    	function quesList(pageOffset,pageSize){
    		 $.ajax({
    	            url:'online_quesList.action?cc='+Math.random(),
    	            type:'get',
    	            ...
    	            success: function(data){
    	                ...
    	                for(var i=0;i<datas.length;i++){
							...
							oLi.onclick=function(){
								window.location.href='online_quesInfo.action?id='+this.id;
							};
							oUl.appendChild(oLi);
    	                }
...
<body>
    <div id="paging"></div>
    <c:if test="${sessionScope.user.role.name=='学生'||sessionScope.user.role.name=='管理员'}">
    	 <input type="button" value="发布问题" id="btn">
    </c:if>
    <!--遮罩层显示问题具体情况-->
    <div id="bg"></div>
    <div id="show">
    ...
    <ul class="list"><li>问题标题<span class="span1">发布者</span><span class="span2">时间</span></li></ul>
</body>
</html>